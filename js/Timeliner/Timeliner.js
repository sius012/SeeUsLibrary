...
$.fn.Timeliner = function (
    ...
    $(this)
        ...
        .each(function (idata) {
            ...
            otherAttribute.each(function () {
                if (
                    ![titleIndex, startIndex, endIndex].includes(
                        $(this).index()
                    )
                ) {
                    let attrName = {};
  ...
  <div class="container-timeline " style="  display: inline-block" data-id="${id}">
    ...
    listOfData.forEach(function (e) {
        let endRange = wrapper
            .find(`div[data-address=${e.end}]`)
            .position().left;
        let startRange = wrapper
            .find(`div[data-address=${e.start}]`)
            .offset().left;
        ...
        let startEl = {
            ...
            monthIndex: $(`div[data-address=${e.start}]`)
                .closest(".col-big")
                .index(),
        };
        let endEl = {
            dateIndex: $(`div[data-address=${e.end}]`).index(),
            monthIndex: $(`div[data-address=${e.end}]`)
                .closest(".col-big")
                .index(),
        };
                ...
                if (e.ctrlKey) {
                    ...
                    window.dataTable[idchart].forEach(function (e, i) {
                        let row = table
                            ...
                            .eq(i)
                            .find(".col-data");
                        let startPos = table.find(
                            "div[data-address=" + e.start + "]"
                        );
                        let startPosData =
                            startPos[0].getBoundingClientRect().left;
                        let endPos = table.find(
                            "div[data-address=" + e.end + "]"
                        );
